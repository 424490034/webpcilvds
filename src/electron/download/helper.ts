...
export const addDownloadItem = async ({
  ...
  const fileName = getFileName(newDownloadItem&&newDownloadItem.fileName || '', item.getFilename())
  const startTime = item.getStartTime() * 1000
  const totalBytes = getBase64Bytes(fileUrl) || item.getTotalBytes()

  let fileId = uuidV4()
  const savePath = newDownloadItem&&newDownloadItem.path || app.getPath('downloads')
