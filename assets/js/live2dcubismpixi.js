...
(function(LIVE2DCUBISMPIXI) {
    var Model = (function(_super) {
        ...
        function Model(coreModel, textures, animator, physicsRig, userData, groups) {
            ...
            for (var m = 0; m < _this._meshes.length; ++m) {
                ...
                _this._meshes[m] = new CubismMesh(textures[_this._coreModel.drawables.textureIndices[m]], _this._coreModel.drawables.vertexPositions[m], uvs, _this._coreModel.drawables.indices[m], window.PIXI.DRAW_MODES.TRIANGLES);
                ...
                if (Live2DCubismCore.Utils.hasBlendAdditiveBit(_this._coreModel.drawables.constantFlags[m])) {
                    if (_this._coreModel.drawables.maskCounts[m] > 0) {
                        var addFilter = new window.PIXI.Filter();
                        addFilter.blendMode = window.PIXI.BLEND_MODES.ADD;
                        _this._meshes[m].filters = [addFilter];
                    } else {
                        _this._meshes[m].blendMode = window.PIXI.BLEND_MODES.ADD;
                    }
                } else if (Live2DCubismCore.Utils.hasBlendMultiplicativeBit(_this._coreModel.drawables.constantFlags[m])) {
                    if (_this._coreModel.drawables.maskCounts[m] > 0) {
                        var multiplyFilter = new window.PIXI.Filter();
                        multiplyFilter.blendMode = window.PIXI.BLEND_MODES.MULTIPLY;
                        _this._meshes[m].filters = [multiplyFilter];
                    } else {
                        _this._meshes[m].blendMode = window.PIXI.BLEND_MODES.MULTIPLY;
                    }
                }
    ...
    var MaskSpriteContainer = (function(_super) {
        ...
        function MaskSpriteContainer(coreModel, pixiModel) {
            ...
            _this._maskShader = new window.PIXI.Filter(_this._maskShaderVertSrc.toString(), _this._maskShaderFragSrc.toString());
            ...
            for (var m = 0; m < pixiModel.meshes.length; ++m) {
                if (_maskCounts[m] > 0) {
                    var newContainer = new window.PIXI.Container;
                    ...
                    var newTexture = window.PIXI.RenderTexture.create(0, 0);
                    _this._maskTextures.push(newTexture);
                    var newSprite = new window.PIXI.Sprite(newTexture);
    ...
    var ModelBuilder = (function() {
        ...
        ModelBuilder.prototype.buildFromModel3Json = function(loader, model3Obj, callbackFunc) {
            ...
            if (typeof(model3Obj.data['FileReferences']['Moc']) !== "undefined")
                loader.add('moc', modelDir + model3Obj.data['FileReferences']['Moc'], { xhrType: window.PIXI.loaders.Resource.XHR_RESPONSE_TYPE.BUFFER });
            ...
            if (typeof(model3Obj.data['FileReferences']['Physics']) !== "undefined")
                loader.add('physics', modelDir + model3Obj.data['FileReferences']['Physics'], { xhrType: window.PIXI.loaders.Resource.XHR_RESPONSE_TYPE.JSON });
            if (typeof(model3Obj.data['FileReferences']['UserData']) !== "undefined")
                loader.add('userdata', modelDir + model3Obj.data['FileReferences']['UserData'], { xhrType: window.PIXI.loaders.Resource.XHR_RESPONSE_TYPE.JSON });
